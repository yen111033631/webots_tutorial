...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.device = torch.device("cuda" if args.cuda else "mps")
    ...
    def save_model(self, env_name, suffix="", actor_path=None, critic_path=None):
        path = '/home/neaf-2080/code/Haw/SAC_reward_check/results/models/' + env_name
        if not os.path.exists(path):
            os.makedirs(path)

        if actor_path is None:
            actor_path = path + "/sac_actor_{}_{}".format(env_name, suffix)
        if critic_path is None:
            critic_path = path + "/sac_critic_{}_{}".format(env_name, suffix)
